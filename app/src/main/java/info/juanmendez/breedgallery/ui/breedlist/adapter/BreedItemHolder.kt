...
class BreedItemHolder(
    var view: BreedListContract.View, var binding: ItemPetBinding
    ...
    @Inject
    @field:RunOn(SchedulerType.COMPUTATION)
    lateinit var computationScheduler: io.reactivex.Scheduler

    @Inject
    @field:RunOn(SchedulerType.UI)
    lateinit var uiScheduler: io.reactivex.Scheduler
    ...
    fun setBreed(breed: Breed) {
        ...
        if(breed.pictureList.isEmpty()) {
            ...
            breedRepository.getPicsByBreed(breed.name).subscribeOn(computationScheduler)
                .observeOn(uiScheduler).bindToLifecycle(view).subscribe({
